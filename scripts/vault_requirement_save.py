...
def vault_requirement_save(
    ...
    if req_type not in REQ_TYPES:
        return {"ok": False, "error": f"req_type '{req_type}' not valid. Use: {REQ_TYPES}"}
    if priority not in PRIORITIES:
        return {"ok": False, "error": f"priority '{priority}' not valid. Use: {PRIORITIES}"}
    ...
    # Status badge
    status_line = " | ".join(
        f"**{s}**" if s == status else s for s in STATUSES
    )
...
""",
    ...
    parser.add_argument("--description", required=True, help="Full requirement description")
    parser.add_argument("--type", dest="req_type", required=True,
                        help=f"Requirement type: {REQ_TYPES}")
    parser.add_argument("--priority", required=True,
                        help=f"MoSCoW priority: {PRIORITIES}")
    parser.add_argument("--acceptance_criteria",
                        help='JSON array of acceptance criteria strings')
    parser.add_argument("--source", help="Origin of the requirement (stakeholder, document, etc.)")
    parser.add_argument("--status", default="draft",
                        help=f"Requirement status (default: draft): {STATUSES}")
    parser.add_argument("--related_code",
                        help="Comma-separated list of related source files")
    ...
    if args.related_code:
        related_code_list = [f.strip() for f in args.related_code.split(",") if f.strip()]

    acceptance_criteria = parse_json_arg(args.acceptance_criteria, "acceptance_criteria")
