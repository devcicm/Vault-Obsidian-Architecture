...
from vault_io import atomic_write_text, assert_within_vault, VAULT_ROOT, safe_wikilink, update_section_index
import uuid
...
def vault_knowledge_save(
    ...
    if category in ["dependency", "framework"]:
        ...
        if related:
            ...
            body_sections.append(f"\n**Por qué se eligió:** " + " ".join(f"[[{safe_wikilink(r)}]]" for r in related) + "\n")
...
""",
    ...
    parser.add_argument("--content", help="Markdown content (optional when --scan-path is used)")
    ...
    parser.add_argument("--scan-path", help="Directory to scan for .md and .txt files and save each as a knowledge note")
    ...
    if args.scan_path:
        ...
        for f in files:
            ...
            if result.get("ok"):
                ...
                skipped.append({"file": str(f), "reason": result.get("error", "unknown")})
