...
def vault_test_save(
    ...
    if test_type not in TEST_TYPES:
        return {"ok": False, "error": f"test_type '{test_type}' not valid. Use: {TEST_TYPES}"}
...
""",
    ...
    parser.add_argument("--test_type", required=True,
                        help=f"Test type: {TEST_TYPES}")
    parser.add_argument("--description", required=True, help="What is being tested")
    parser.add_argument("--preconditions", help="Setup conditions required before running the test")
    parser.add_argument("--steps",
                        help='JSON array of steps: [{"step":1,"action":"...","expected":"..."}]')
    parser.add_argument("--expected_result", help="Overall expected outcome")
    parser.add_argument("--related_requirement",
                        help="Related requirement ID (e.g. REQ-001)")
