...
def save_infra_map(components: List[Dict], project: Optional[str] = None, location: Optional[str] = None) -> Path:
...
def vault_infra_save(
    ...
    if component_type not in COMPONENT_TYPES:
        return {"ok": False, "error": f"Tipo inválido: {type}. Válidos: {list(COMPONENT_TYPES.keys())}"}
...
""",
    ...
    parser.add_argument("--type", required=True, help=f"Component type: {list(COMPONENT_TYPES.keys())}")
