...
reason: Script paraguas reemplazado por la CLI consolidada.
"""
...
class VaultTools:
    ...
    def diff(self, note_path: str):
        ...
        versions = sorted(
            history_dir.glob(f"*{note_name}*.md"),
        ...
        return {
            "current": str(current.relative_to(self.vault)),
    ...
    def graph(self):
        ...
        for md in self.vault.rglob("*.md"):
            ...
            graph["nodes"].append(
                {"id": md.stem, "path": str(md.relative_to(self.vault))}
            )
    ...
    def backup(self, dest: str = None):
        ...
        if not dest:
            dest = f"vault-backup-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}.zip"
    ...
    def clean_orphans(self):
        ...
        all_notes = {
            n.stem for n in self.vault.rglob("*.md") if ".history" not in str(n)
        }
...
def main():
    ...
    print(
        json.dumps(
            {"_deprecation": {"use_instead": "individual tools", "since": "v26"}}
        ),
        file=sys.stderr,
    )
...
""",
    ...
    parser.add_argument(
        "--limit", "-l", type=int, default=20, help="Limite de resultados"
    )
