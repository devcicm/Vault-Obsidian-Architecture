...
superseded_by: vault_write.py
reason: La creacion de notas se unifico con la escritura (guards, .history, ledger AP-37).
"""
...
def main():
    ...
    print(
        json.dumps({"_deprecation": {"use_instead": "vault_write", "since": "v26"}}),
        file=sys.stderr,
    )
...
""",
    ...
    parser.add_argument(
        ...
        "-f",
        default="01_Projects",
        help="Carpeta destino (e.g. 01_Projects/mi-api)",
    )
    ...
    parser.add_argument(
        "--template", "-m", choices=["project", "runbook", "adr"], help="Template"
    )

    parser.add_argument(
        "--status-default", default="planificado", help="Status para ADR"
    )
