...
superseded_by: vault_move.py
reason: El movimiento actualiza wiki-links, search-index.json y graph.json.
"""
...
def reorganize(project: str = "mi-proyecto", dry_run: bool = False):
    ...
    for note in migrated_root.rglob("*.md"):
        ...
        slug = re.sub(
            r"^(direct|indirect|agents|guides|deployment|architecture|project|informacion.decrepita)-",
            "",
            slug,
        )
...
def regenerate_indexes():
    ...
    search_index = {
        "version": "1.0",
        "updatedAt": datetime.now(timezone.utc).isoformat()[:19],
        "notes": [],
    }
    ...
    (VAULT_ROOT / "99_Index/graph.json").write_text(
        json.dumps(graph, indent=2), encoding="utf-8"
    )

    (VAULT_ROOT / "99_Index/search-index.json").write_text(
...
""",
    ...
    parser.add_argument(
        "--project",
        default="mi-proyecto",
        help="Slug del proyecto (fallback destino, default: mi-proyecto)",
    )

    parser.add_argument(
        "--dry-run", action="store_true", help="Mostrar movimientos sin ejecutarlos"
    )
    ...
    if args.action == "reorganize":
        ...
        print(
            f"{'[DRY-RUN] ' if args.dry_run else ''}Moved: {len(moved)} | Errors: {len(missing)} | Skipped: {len(skipped)}"
        )
