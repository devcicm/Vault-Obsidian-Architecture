    ...
    if rebuild_graph and not dry_run:
        try:
            ...
            if graph_script.exists():
                ...
                result["graph"] = graph_result.get("stats", {"error": proc.stderr[:200]})
...
""",
    ...
    parser.add_argument("--check", action="store_true", help="Exit 0 if index OK, 1 if empty/missing")
    ...
    if args.check:
        ...
        if ok:
            ...
            print(json.dumps({"ok": False, "status": "index_empty_or_missing", "action": "run vault_reindex.py"}))
            return 1
