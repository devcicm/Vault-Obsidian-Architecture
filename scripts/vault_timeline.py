...
def search_sessions(query: str, date_from: str, date_to: str, limit: int) -> List[Dict[str, Any]]:
    events = []
...
def search_changelog(query: str, project: str, date_from: str, date_to: str, limit: int) -> List[Dict[str, Any]]:
    events = []
    for base_path in CHANGELOG_PATHS:
        ...
        for project_folder in project_path.iterdir():
            ...
            for match in re.finditer(
                r"^### (v?\d+\.\d+[^ ]*) — (\d{4}-\d{2}-\d{2})(.*?)(?=^### |\Z)", content, re.DOTALL | re.MULTILINE
            ):
...
def search_notes(
    query: str, sources: List[str], date_from: str, date_to: str, project: str, limit: int
) -> List[Dict[str, Any]]:
    try:
    ...
    for note in index.get("notes", []):
        ...
        if "03_Decisions/" in path and "decisions" in sources:
            ...
            source = "knowledge"
        elif ("07_Knowledge/dependencies/" in path or "07_Knowledge/frameworks/" in path) and "dependencies" in sources:
            source = "dependencies"
...
def vault_timeline(
    ...
    if any(s in sources for s in ["decisions", "errors", "patterns", "infra", "knowledge", "dependencies", "runbooks"]):
        events.extend(search_notes(query, sources, from_date, to_date, project, limit))

    events.sort(key=lambda x: (x["date"], 0 if x["type"] == "updated" else 1), reverse=True)
    events = events[:limit]
