...
def _tag_frontmatter(content: str, source: str) -> str:
    ...
    tag_lines = [
        f"mergedFrom: {source}",
        f"mergedAt: {datetime.now().isoformat()[:19]}Z",
    ]
...
def _action_merge(source_dir: Path, conflict: str) -> Dict[str, Any]:
    notes = [
        n for n in source_dir.rglob("*.md")
...
def vault_merge(
    ...
    return {"ok": False, "error": f"Unknown action: {action}. Use merge | detect | dedup"}
...
""",
    ...
    parser.add_argument("--source", help="Path to external vault root (required for action=merge)")
    parser.add_argument(
        "--conflict",
