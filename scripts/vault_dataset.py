...
def search_keywords(
    query: str, operator: str = "OR", folder: str = None, min_score: float = 0, top: int = 20, mode: str = "nltk"
):
...
def list_keywords(limit: int = 30):
    ...
    for kw, info in sorted(keywords.items(), key=lambda x: -x[1].get("count", 0))[:limit]:
        print(f"  {kw}: {info.get('count', 0)} files, weight: {info.get('weight', 0):.2f}")
...
""",
    ...
    parser.add_argument("command", choices=["extract", "search", "keywords", "index"], help="Comando")
    ...
    parser.add_argument("--operator", "-o", choices=["AND", "OR", "NOT"], default="OR", help="Operador logico")
    ...
    if args.command == "extract":
        ...
        for r in results:
            print(f"\n[{r['title']}]")
            print(f"  {r['path']}")
            print(f"  Score: {r.get('score', 0):.3f}, Matches: {r.get('match_count', 0)}")
