...
DATA_SUBJECTS = ["customers", "employees", "contractors", "minors", "prospects", "partners"]
...
def vault_privacy_save(
    ...
    if legal_basis not in LEGAL_BASES:
        return {"ok": False, "error_code": "INVALID_LEGAL_BASIS",
    ...
    third_md = "\n".join(
        f"| `{tp}` | — | Pendiente de DPA | — |"
        for tp in third_parties
    ) or "| — | — | — | — |"

    dpia_text = "**Requerida** — iniciar DPIA antes de procesar datos" if dpia_required else "No requerida (verificar si aplica)"
...
""",
    ...
    parser.add_argument("--legal_basis", required=True, choices=list(LEGAL_BASES.keys()))
    ...
    parser.add_argument("--transfers_outside_eu", type=lambda x: x.lower() == "true", default=False)
    parser.add_argument("--dpia_required",  type=lambda x: x.lower() == "true", default=None)
    ...
    result = vault_privacy_save(
        project=args.project, title=args.title, purpose=args.purpose,
        legal_basis=args.legal_basis, pii_categories=pii,
        retention_period=args.retention_period, data_subjects=subj,
        third_parties=thirds, transfers_outside_eu=args.transfers_outside_eu,
        dpia_required=args.dpia_required, controller=args.controller,
        processor=args.processor, status=args.status, agent=args.agent,
    )
