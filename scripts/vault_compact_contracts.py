...
_GROUPS_HARDCODED: List[Dict[str, Any]] = [
    ...
    {"id": 4,  "name": "Diagramas",     "tools": ["vault_diagram_save","vault_relation_add"]},
    {"id": 5,  "name": "Conocimiento",  "tools": ["vault_knowledge_save","vault_knowledge_get"]},
    {"id": 6,  "name": "Salud",         "tools": ["vault_audit","vault_validate","vault_graph"]},
    {"id": 7,  "name": "Runbooks",      "tools": ["vault_runbook_save","vault_runbook_log"]},
    {"id": 8,  "name": "Infraestructura","tools":["vault_infra_save","vault_infra_map","vault_env_save"]},
    {"id": 9,  "name": "Migración",     "tools": ["vault_migrate_docs","vault_migrate_rollback"]},
    {"id": 10, "name": "Línea de tiempo","tools":["vault_timeline"]},
    {"id": 11, "name": "Vista proyecto", "tools":["vault_project_status","vault_project_overview"]},
    {"id": 12, "name": "Código",        "tools": ["vault_code_module","vault_code_relation","vault_code_map","vault_code_query"]},
    {"id": 13, "name": "Backups",       "tools": ["vault_backup","vault_backup_list","vault_restore"]},
    {"id": 14, "name": "Seguridad",     "tools": ["vault_security_scan"]},
    {"id": 15, "name": "Índices",       "tools": ["vault_section_index","vault_master_index","vault_reindex"]},
...
PROFILES: Dict[str, List[str]] = {
    "minimal": [
        "vault_write", "vault_read", "vault_search", "vault_list", "vault_append",
        "vault_audit", "vault_reindex", "vault_change_log", "vault_standard_upgrade",
        "vault_errors",
    ],
    "standard": [
        "vault_write", "vault_read", "vault_search", "vault_list", "vault_append",
        "vault_diff", "vault_merge", "vault_validate", "vault_graph", "vault_log_error",
        "vault_pattern_save", "vault_pattern_list", "vault_knowledge_save", "vault_knowledge_get",
        "vault_diagram_save", "vault_relation_add", "vault_runbook_save", "vault_runbook_log",
        "vault_infra_save", "vault_env_save", "vault_project_status", "vault_project_overview",
        "vault_code_module", "vault_backup", "vault_backup_list", "vault_restore",
        "vault_section_index", "vault_master_index", "vault_reindex", "vault_drift_detect",
        "vault_security_scan", "vault_timeline", "vault_bibliography_save", "vault_flow_save",
        "vault_migrate_docs", "vault_migrate_rollback", "vault_change_log",
        "vault_standard_upgrade", "vault_audit",
    ],
}
...
def _extract_argparse_args(source: str) -> tuple:
    ...
    while i < len(lines):
        ...
        if "add_argument(" in line:
            ...
            arg_type = type_match.group(1) if type_match else ("bool" if is_store_true else "str")
...
def _resolve_tool_list(profile: str) -> List[str]:
    ...
    all_scripts = sorted(
        ...
        if p.stem not in ("vault_errors", "vault_test_runner", "vault_compact_contracts", "vault_manifest")
    )
    return all_scripts
...
def _contracts_to_md(contracts: List[Dict[str, Any]], profile: str) -> str:
    ...
    for c in contracts:
        if c["group"] != current_group:
            ...
            lines.append("| Tool | Descripción | Args requeridos | Args opcionales |")
            lines.append("|---|---|---|---|")

        req = ", ".join(f"`{a['flag']}`" for a in c["required_args"]) or "—"
        opt_count = len(c["optional_args"])
        opt = f"{opt_count} opcional{'es' if opt_count != 1 else ''}" if opt_count else "—"
        dep_tag = " *(deprecated)*" if c.get("deprecated") else ""
        lines.append(f"| `{c['name']}`{dep_tag} | {c['description'][:80]} | {req} | {opt} |")
...
""",
    ...
    parser.add_argument("--check", action="store_true", help="Solo mostrar contratos, no escribir archivos")
