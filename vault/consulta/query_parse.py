...
def _sections(available_sections: Optional[List[str]] = None) -> List[str]:
    """Devuelve secciones inyectadas o las que esta gramática puede reconocer.

    La implementación instalada no importa ``vault_registry``: el registro es
    un adaptador legacy del checkout. La fachada histórica le inyecta las
    secciones canónicas; sin esa inyección el parser sigue siendo útil y
    determinista para las pistas que posee.
    """
    return list(available_sections) if available_sections is not None else sorted(SECTION_HINTS)
...
def vault_query_parse(
    ...
    *,
    ...
    sections: List[str] = []
    valid_sections = set(_sections(available_sections))
...
def main(*, available_sections: Optional[List[str]] = None) -> int:
...
""",
    ...
    args.query = pregunta
    result = vault_query_parse(args.query, available_sections=available_sections)
