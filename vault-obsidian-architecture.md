...
1. **El audit no lo ejecuta nadie.** En las **1.356 ejecuciones de tools
   ...
   aparece **ni una vez**. 41 de las 117 tools del catálogo no se han ejecutado
...
camino que un agente real usa**. `vault_smoke` recorre las 117 tools del catálogo,
...
**Adapter de modelo LLM: el vault se adapta al que conecta.**

El problema: `vault_context_pack` usaba un budget hardcodeado de 4000 tokens sin conocer la ventana del modelo. La solución: auto-detección del perfil desde `clientInfo.name` del handshake MCP, registry de perfiles en `model_profiles.json`, y budget dinámico consumido por `vault_context_pack`.

- Nueva tool `vault_model_profile`: `--list`, `--active`, `--set`, `--auto`, `--budget`, `--window`
- Perfiles: claude/cursor (15000), gpt-4o (8000), deepseek (6000), gemini (4000), floor 2000
- MCP servidor: `clientModelProfile` auto-detectado desde nombre del cliente
- `VAULT_MODEL_PROFILE` propagada como env var a subprocess
- Fix: encoding UTF-8 en subprocess, lectura de entorno via `leer()` no `os.environ`
- Fix: sincronización de `env-table.json`, `field-compat-baseline.json`, `docs/BLUEPRINT.md`
